...
| 07 | [PaperOut To-Authors 使用指南](07-PaperOut%20To-Authors%20使用指南.md) · [协作与使用场景](07-PaperOut%20协作与使用场景.md) | 怎样组织长文并完成正式输出 | 建立写作项目并导出 PDF 或 Word |{>>这一行塞了两篇文档，序号也都是 07。两篇的定位其实差得很远——「使用指南」是动手操作，「协作与使用场景」是套件定位和路线图（里面还有 🔵 未实现 的条目）。新手照着「阅读顺序」读到 07，会一头撞进路线图。建议拆成两行并把后者改成 08，或者干脆把「协作与使用场景」移出教程正文、放到 01-产品介绍 那边去<<}
