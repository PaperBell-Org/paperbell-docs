...
| 压力 | 说的是什么 | 对应 |
| :---: | --- | :---: |
| 📥 | 信息时代，材料和参考资料**唾手可得**，多到淹没你自己 | **I**nput |
| 🧠 | 概念是信息之上的高度抽象，是**人脑存储信息的原子单元** | **C**oncept |
...
> 4. **元数据 ⇄ 全局**：日记（时间）、学者和机构（人 / 地）这些流水账，被关键词检索、被精选关键词整合，既供写作时调取素材，也服务项目管理（合作者、评审专家……）。
...
> [!info] 谁来维护这一层
> **PaperBell Cards Wrangler 插件**：读取输入笔记、用 LLM 把关键词/实体沉淀成词条、按 `aliases` 合并变体、维护双链与索引。精选与降级、是否回写关键词到输入（关 / 逐条确认 / 自动，共三档），都由你掌控。
